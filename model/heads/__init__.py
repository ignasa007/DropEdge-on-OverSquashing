from model.heads.node_c import NodeClassification
...
def verify_task(dataset_name, task_name):
    ...
    if formatted_name not in valid_tasks.get(dataset_name):
        raise ValueError(f'Parameter `task_name` not recognised for the passed `dataset_name` (got task `{task_name}` for dataset `{dataset_name}`).')
